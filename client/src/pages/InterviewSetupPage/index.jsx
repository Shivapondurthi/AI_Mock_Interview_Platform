...
function InterviewSetupPage() {
  ...
  useEffect(() => {
    const loadResume = async () => {
      try {
        const data = await interviewService.getResume();
        if (data) {
  ...
  const handleStartInterview = async () => {
    ...
    setLoading(true);
    try {
      const totalQuestions = DIFFICULTY_TO_QUESTIONS[selectedDifficulty] || 5;
