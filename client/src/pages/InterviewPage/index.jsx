...
function InterviewPage() {
  ...
  useEffect(() => {
    const loadInterview = async () => {
      try {
        if (location.state?.greetingAudio) {
          setCurrentAudio(location.state.greetingAudio);
          setInterviewerText(location.state.greeting || '');
          setCurrentQuestionNum(location.state.currentQuestion || 1);
          setTotalQuestions(location.state.totalQuestions || 5);
          if (location.state.question) {
            setCurrentQuestion({ text: location.state.question, type: 'intro', isCodeQuestion: false });
          }
          setLoading(false);
        } else {
          ...
          setCurrentAudio(data.lastAudio || null);
          setInterviewerText(data.latestInterviewerMessage || '');
  ...
  const processAnswerResult = (result) => {
    ...
    if (result.isComplete) {
      ...
      setFarewellMessage('Thank you for completing this interview. Your feedback is being generated...');
  ...
  const submitAndProcess = async (answerText) => {
    ...
    try {
      const result = await interviewService.submitTextAnswer(id, answerText.trim());
  ...
  const handleRecordingComplete = async (audioBlob) => {
    ...
    try {
      const result = await interviewService.submitVoiceAnswer(id, audioBlob);
      processAnswerResult(result);
    } catch (error) {
      setSubmitting(false);
      setInterviewerState(STATE_LISTENING);
      toast.error(error.response?.data?.message || 'Failed to process voice answer');
    }
  };
  ...
  const handleSubmitCode = async () => {
    if (!code || !code.trim()) {
      toast.error('Please write or modify the code');
      return;
    }
    ...
    try {
      const result = await interviewService.submitCode(id, code.trim(), codeLanguage);
  ...
  const handleEndInterview = async () => {
    ...
    try {
      const result = await interviewService.endInterview(id);
          ...
          <>
            ...
            {isCodeQuestion && (
              <div className="code-answer-block">
                    ...
                    <div className="text-fallback-block">
                      ...
                      {showTextFallback && (
                        <div className="text-answer-block">
                          ...
                          />
                          <button
                            className={`submit-text-btn ${submitting || !code.trim() ? 'submit-text-btn-disabled' : ''}`}
                            onClick={handleSubmitCode}
                            disabled={submitting || !code.trim()}
                          >
                ...
                {codeEvaluation && (
                  <div className="code-evaluation-box">
                    <div className="code-eval-header">
                      <span className="code-eval-title">Evaluation</span>
                      <span className="code-eval-status-icons">
