...
function HomePage() {
  ...
  useEffect(() => {
    const loadInterviews = async () => {
      try {
        const data = await historyService.getInterviewHistory(1, 100);
  ...
  const handleDelete = async (interviewId) => {
    try {
      ...
      setAllInterviews((prev) => prev.filter((i) => i._id !== interviewId));
      setRecentInterviews((prev) => prev.filter((i) => i._id !== interviewId));
